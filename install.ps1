...
function server_download {
    if ($javaInstalled) {
        if (Test-Path -Path $server_directory -PathType Container) {
            ...
            Remove-Item -Recurse -Force -Path $server_directory
...
function server_setup {
    ...
    Copy-Item -Path .\start.ps1 -Destination $server_directory
    Copy-Item -Path .\uninstall.ps1 -Destination $server_directory
}
if ($java_installed) {
    server_setup
}
